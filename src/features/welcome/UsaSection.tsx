...
const UsaSection = () => {
  ...
  if (loading) return <div className="loading">Loading USA data…</div>;

  return (
    <section className="usa-inline-section">
      <div className="section-header">
        <h2>Data Science in the United States</h2>
        <p>Job market breakdown, experience level analysis, and salary correlations (2020–2025)</p>
      </div>

      {bubbleData.length > 0 && (
        <div className="panel">
          <h3>Top Job Titles by Popularity</h3>
      ...
      {expBarData.length > 0 && (
        <div className="panel">
          <h3>Median Salary by Experience Level</h3>
      ...
      {correlationCharts.length > 0 && (
        <div className="panel">
          <h3>Salary &amp; Macro Indicator Trends</h3>
          <p className="chart-caption">
          ...
          <div className="usa-correlation-grid">
            {correlationCharts.map((c) => (
              <div key={c.label} className="panel chart-panel">
                <h4>Salary vs {c.label}</h4>
                <p className="chart-caption">
                  Both indexed to 100 in the first available year. Correlation:{" "}
                  <strong>{rLabel(c.r)}</strong> (r = {c.r.toFixed(2)})
                </p>
      ...
      <div style={{ marginTop: 16, fontSize: 13, textAlign: "right" }}>
        <button className="secondary-button" onClick={() => navigate("/country/USA")}>
          View USA country profile →
        </button>
      </div>
    </section>
  );
